...
static void ngx_http_upload_finish_handler(ngx_http_upload_ctx_t *u) { /* {{{ */
    ...
    if(u->is_file) {
        ucln = u->cln->data;
#if (NGX_WIN32)  // modified by chnykn 2022.02.01
        ucln->fd = NULL;
#else 
        ucln->fd = -1;
#endif
...
//add by chnykn 2022.02.01
static long ngx_file_st_size(ngx_file_info_t info) {
#if (NGX_WIN32) // modified by chnykn 2022.02.01
    ULARGE_INTEGER ul = { 0 };
...
ngx_http_upload_merge_ranges(ngx_http_upload_ctx_t *u, ngx_http_upload_range_t *range_n) {
    ...
    do {
		...
		//modified by chnykn
        if(state_file->offset < ngx_file_st_size(state_file->info)) {
            remaining = ngx_file_st_size(state_file->info) - state_file->offset > in_buf.end - in_buf.start
                ? in_buf.end - in_buf.start : ngx_file_st_size(state_file->info) - state_file->offset;
		...
		//modified by chnykn
        in_buf.last_buf = state_file->offset == ngx_file_st_size(state_file->info) ? 1 : 0;
    ...
    } while(state_file->offset < ngx_file_st_size(state_file->info));
    ...
    if(out_buf.file_pos < ngx_file_st_size(state_file->info)) {
